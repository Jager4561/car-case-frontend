...
const { reportPost } = usePostsState();
...
const sendReport = async () => {
  ...
  if(reason.value.length === 0) {
    createWarningToast('Zgłaszanie instrukcji', 'Powód zgłoszenia nie może być pusty.');
    return;
  } 
  isFormPending.value = true;
  try {
    await reportPost(props.options.postId, reason.value);
    createSuccessToast('Zgłaszanie komentarza', 'Instrukcja naprawy została zgłoszona. Dziękujemy');
    reason.value = '';
    close();
  } catch (error) { 
    createErrorToast('Zgłaszanie instrukcji', 'Wystąpił błąd podczas zgłaszania instrukcji naprawy. Spróbuj ponownie później.');
  } finally {
...
<template>
  <form class="comment-login-popup" @submit.prevent="sendReport()">
    <div class="header">
      <div class="title">Zgłoś instrukcję naprawy</div>
      <div class="header-actions">
        <button type="button" class="icon-button icon-button__medium icon-button__secondary" @click="close()">
          <XMarkIcon class="button-icon"></XMarkIcon>
        </button>
      </div>
    </div>
    <div class="content">
      <p class="description">Wpisz powód zgłoszenia instrukcji np. (obrażanie innych użytkowników, wprowadzanie w błąd itp.)</p>
